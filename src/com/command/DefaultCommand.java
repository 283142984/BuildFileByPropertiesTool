...
public class DefaultCommand {
    ...
    public static StringBuffer foreachCommand(Map<Integer, FieldBean> fieldBeanMap, StringBuffer fileStringBuffer, int useNumber) {
        ...
        for (; useNumber > 0; useNumber--) {
            ...
            tplString = OtherUtils.replaceFirstString(tplString, "\t,\r,\n", "", 0);
            for (Map.Entry<Integer, FieldBean> entry : fieldBeanMap.entrySet()) {
                ...
                if (fieldBean.getFieldCode() != null && fieldBean.getFieldName() != null) {
                    ...
                    String upperFieldName = fieldName.substring(0, 1).toUpperCase();
                    if (fieldName.length() > 1) {
                        upperFieldName = upperFieldName + fieldName.substring(1, fieldName.length());
                    }
                    String upperFieldCode = fieldCode.substring(0, 1).toUpperCase();
                    if (fieldCode.length() > 1) {
                        upperFieldCode = upperFieldCode + fieldCode.substring(1, fieldCode.length());
                    }
